...
class AutoDeployRouter(RabbitMQClient, Database):
  ...
  def setup(self, schemas):
    ...
    # input dtype.
    self.input_dtype = self.internal_config.PREDICT_INPUT_DTYPE
  ...
  def _build_predict_url(self, input_hash_id, _input_array):
    _PREDICT_URL = f'http://{self.internal_config.PREDICT_URL}:{self.internal_config.PREDICT_PORT}/model_predict'
    return f'{_PREDICT_URL}?id={input_hash_id}&ndim={len(_input_array.shape)}'
  ...
  def register_router(self):
    ...
    async def api_endpoint(payload: input_model_schema.UserInputSchema, token: Optional[Union[Text, Any]] = Depends(oauth2_scheme) if _protected else None):
      nonlocal self
      try:
        _input_array = []
